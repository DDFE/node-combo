...
var Server = function (root, options) {
	...
	if ('serverInfo' in this.options) {
		...
		this.serverInfo = 'node-combo/' + version.join('.');
	}
...
Server.prototype.serveCombo = function (req, res, config, callback) {
	var comboFn = combo(config, true);
	comboFn(req, res, callback);
};
